...
def threshold_detection(model, loader):
    all_loss = []
    true_labels = []
...
def train(**kwargs):
    ...
    criterion = nn.MSELoss()
    optimizer = torch.optim.Adam(opt.model.parameters(), lr=opt.optim_lr)
    ...
    for i in range(opt.epoch):
        ...
        loop.set_description(f"Training [{i}/{opt.epoch}]")
        running_loss = 0.0
        model.train()
        for data in loop:
            image = data[0].cuda(0)
        ...
        with torch.no_grad():
            ...
            loop.set_description(f"Validation [{i}/{opt.epoch}]")
            for data in loop:
                image = data[0].cuda(0)
                output = model(image)
        ...
        threshold = threshold_detection(model, valid_loader)

        with open(f"threshold for {model._get_name()}.txt", "w") as output:
            output.write((str(threshold)))

        #TODO: visualization.train_val_plot(model._get_name(), epoch_loss_train, epoch_loss_valid)
...
def test(**kwargs):
    ...
    with open(f"threshold for {model._get_name()}.txt") as f:
